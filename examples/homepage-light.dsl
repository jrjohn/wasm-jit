...
// ---- the condition (緣) the host hands in: the fraction of w reseeds the
// ---- constellation — reload the page and this sky is born again ----
let rs = (w % 1.0) * 173.7;
...
let i = 0.0;
while i < N {
  let ali = sin((i + rs) * 12.9898) * 437.585;
ali = ali % 1.0;
if ali < 0.0 { ali = ali + 1.0; }
let rli = sin((i + rs) * 78.233) * 125.432;
rli = rli % 1.0;
if rli < 0.0 { rli = rli + 1.0; }
let dli = (0.05 + 0.6 * rli) * S;
if (i % 2.0) > 0.5 { dli = (0.28 + 0.67 * rli) * S; }
...
yi = yi + cos(i * 2.3 + t * (0.11 + (i % 3.0) * 0.05)) * (6.0 + (i % 3.0) * 3.0);
  let j = i + 1.0;
  while j < N {
    let alj = sin((j + rs) * 12.9898) * 437.585;
alj = alj % 1.0;
if alj < 0.0 { alj = alj + 1.0; }
let rlj = sin((j + rs) * 78.233) * 125.432;
rlj = rlj % 1.0;
if rlj < 0.0 { rlj = rlj + 1.0; }
let dlj = (0.05 + 0.6 * rlj) * S;
if (j % 2.0) > 0.5 { dlj = (0.28 + 0.67 * rlj) * S; }
...
yj = yj + cos(j * 2.3 + t * (0.11 + (j % 3.0) * 0.05)) * (6.0 + (j % 3.0) * 3.0);
    ...
    let df = (dli + dlj) / (2.0 * S);
    let DD = D * (0.66 + df * 0.95);
    if q < DD * DD {
      let cl = 1.0 - q / (DD * DD);
      hsl(0.556, 0.26, 0.845 + cl * -0.105);
      line(xi, yi, xj, yj);
    }
    j = j + 1.0;
  }
...
let k = 0.0;
while k < N {
  let abk = sin((k + rs) * 12.9898) * 437.585;
abk = abk % 1.0;
if abk < 0.0 { abk = abk + 1.0; }
let rbk = sin((k + rs) * 78.233) * 125.432;
rbk = rbk % 1.0;
if rbk < 0.0 { rbk = rbk + 1.0; }
let dbk = (0.05 + 0.6 * rbk) * S;
if (k % 2.0) > 0.5 { dbk = (0.28 + 0.67 * rbk) * S; }
...
let apa1 = sin((na1 + rs) * 12.9898) * 437.585;
apa1 = apa1 % 1.0;
if apa1 < 0.0 { apa1 = apa1 + 1.0; }
let rpa1 = sin((na1 + rs) * 78.233) * 125.432;
rpa1 = rpa1 % 1.0;
if rpa1 < 0.0 { rpa1 = rpa1 + 1.0; }
let dpa1 = (0.05 + 0.6 * rpa1) * S;
if (na1 % 2.0) > 0.5 { dpa1 = (0.28 + 0.67 * rpa1) * S; }
...
let apb1 = sin((nb1 + rs) * 12.9898) * 437.585;
apb1 = apb1 % 1.0;
if apb1 < 0.0 { apb1 = apb1 + 1.0; }
let rpb1 = sin((nb1 + rs) * 78.233) * 125.432;
rpb1 = rpb1 % 1.0;
if rpb1 < 0.0 { rpb1 = rpb1 + 1.0; }
let dpb1 = (0.05 + 0.6 * rpb1) * S;
if (nb1 % 2.0) > 0.5 { dpb1 = (0.28 + 0.67 * rpb1) * S; }
...
let vx1 = xb1 - xa1;
let vy1 = yb1 - ya1;
let df1 = (dpa1 + dpb1) / (2.0 * S);
let DL1 = D * (0.66 + df1 * 0.95) * 0.95;
if (vx1 * vx1 + vy1 * vy1) < DL1 * DL1 {
  hsl(0.56, 0.60, 0.42);
  glow(xa1 + vx1 * pp1, ya1 + vy1 * pp1, 8.0);
  disc(xa1 + vx1 * pp1, ya1 + vy1 * pp1, 1.8);
}
let apa2 = sin((na2 + rs) * 12.9898) * 437.585;
apa2 = apa2 % 1.0;
if apa2 < 0.0 { apa2 = apa2 + 1.0; }
let rpa2 = sin((na2 + rs) * 78.233) * 125.432;
rpa2 = rpa2 % 1.0;
if rpa2 < 0.0 { rpa2 = rpa2 + 1.0; }
let dpa2 = (0.05 + 0.6 * rpa2) * S;
if (na2 % 2.0) > 0.5 { dpa2 = (0.28 + 0.67 * rpa2) * S; }
...
let apb2 = sin((nb2 + rs) * 12.9898) * 437.585;
apb2 = apb2 % 1.0;
if apb2 < 0.0 { apb2 = apb2 + 1.0; }
let rpb2 = sin((nb2 + rs) * 78.233) * 125.432;
rpb2 = rpb2 % 1.0;
if rpb2 < 0.0 { rpb2 = rpb2 + 1.0; }
let dpb2 = (0.05 + 0.6 * rpb2) * S;
if (nb2 % 2.0) > 0.5 { dpb2 = (0.28 + 0.67 * rpb2) * S; }
...
let vx2 = xb2 - xa2;
let vy2 = yb2 - ya2;
let df2 = (dpa2 + dpb2) / (2.0 * S);
let DL2 = D * (0.66 + df2 * 0.95) * 0.95;
if (vx2 * vx2 + vy2 * vy2) < DL2 * DL2 {
  hsl(0.56, 0.60, 0.42);
