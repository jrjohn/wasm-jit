...
The measurement harness is **already live, not planned**: every generation on the public deployment records first-attempt validity, repair rounds, input/output/**cache-read** tokens, latency, and cost. Even the small-n live signal already sharpens the design and is worth stating for honesty — contract caching is effective (**~95% of input tokens are served from cache**, so *prompt length is not the cost or latency bottleneck*), and end-to-end latency (**~9 s median**) is dominated by inference and output length, not context. That directs Study 2's reliability–cost frontier at the variables that actually move it — **vocabulary size and output length**, not prompt size — and it means the study scales an instrumented harness that is already running, rather than standing one up from scratch.
