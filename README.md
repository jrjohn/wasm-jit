...
**Where this sits (related work, honestly).** No single ingredient here is new, and this isn't a new sandbox primitive. Capability-based WASM sandboxing of AI-generated code is a crowded, active space (NVIDIA, Cosmonic, the WASI model); runtime DSL→WASM via `wasm-encoder` is a known compiler pattern; "a small language safe *by construction*" has a named academic instance ([Anvil](https://arxiv.org/pdf/2512.25065) — restricted scalars/arithmetic/bounded-loops, no pointers/arrays/heap/recursion); constraining UI generation to a fixed vocabulary is studied too ([UI-grammar-guided generation](https://arxiv.org/pdf/2310.15455)). The field calls the product side *generative UI*: most tools (v0, bolt, Lovable) have the model emit ordinary code you run as-is — their sandbox guarantees don't survive deployment — and the closest research cousin, [Renderify](https://dev.to/unadlib/renderify-a-runtime-engine-for-rendering-llm-generated-ui-instantly-in-the-browser-1amf), sandboxes LLM-generated JSX via *seven layered defenses*. wasm-jit's bet is the **composition none of them make together**: the interface is *composed from a fixed capability vocabulary, never arbitrary code*, compiled into a cell where **the import table IS the boundary** — one structural invariant (safe by construction, not stacked mitigations), *independent of the generator's intelligence*, *monotonically attenuating across recursive generation*. Synthesis and framing over a real gap — delivered open, reproducible, and one-minute-falsifiable. That is the whole bet.
